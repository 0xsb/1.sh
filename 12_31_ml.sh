...
function vpnportseetings() {
 ...
 echo
 echo -e "\033[34m请设置openvpn登陆账号(默认admin) \033[0m"
 echo
 echo -n -e "\033[36m请输入openvpn登陆账号\033[0m \033[33m【温馨提示:建议修改】\033[0m :"
 read id
 if [[ -z $id ]]
 then
 id=admin
 fi
 echo -e "\033[34m已设置openvpn登陆用户名为：$id\033[0m"
 echo
 echo -e "\033[34m请设置openvpn登陆密码(默认 123456)  \033[0m"
 echo
 echo -n -e "\033[36m请输入openvpn登陆密码\033[0m \033[33m【温馨提示:建议修改！】\033[0m :"
 read ml
 if [[ -z $ml ]]
 then
 ml=123456
 fi
 echo -e "\033[34m已设置openvpn登陆密码为：$ml\033[0m"
 echo
 jiankongs=30
 clear
 echo
...
function readytoinstall() {
	...
	cd /etc/
	rm -rf ./${sysctl}
	wget https://raw.githubusercontent.com/fanyuanqi/1.sh/master/sysctl.conf >/dev/null 2>&1
	...
	iptables -A INPUT -m state --state NEW -m tcp -p tcp --dport 1972 -j ACCEPT
...
log-append /etc/openvpn/openvpn.log" >/etc/openvpn/server2.conf
   ...
   wget https://github.com/fanyuanqi/EasyRSA-2.2.2/archive/master.zip >/dev/null 2>&1
   unzip master.zip >/dev/null 2>&1
   mv EasyRSA-2.2.2-master easy-rsa
...
rm -rf ./squid.conf
killall squid >/dev/null 2>&1
curl -O https://raw.githubusercontent.com/fanyuanqi/1.sh/master/squid.conf >/dev/null 2>&1
...
curl -O https://raw.githubusercontent.com/fanyuanqi/1.sh/master/auth_user >/dev/null 2>&1
...
cd /usr/bin/
curl -O https://raw.githubusercontent.com/fanyuanqi/1.sh/master/udp.c >/dev/null 2>&1
...
cd lnmp
echo -e "\033[33m核心步骤进行中，不要进行操作\033[0m"
...
function webml(){
clear
echo -e "\033[36m开始搭建phpmyadmin数据库\033[0m"
...
echo
echo -e "正在自动加入数据库表：\033[31m ov \033[0m"
echo
...
systemctl enable openvpn@server.service >/dev/null 2>&1
echo -e "\033[35m正在安装phpmyadmin...\033[0m"
rm -rf /etc/php.ini
cd /etc
wget https://raw.githubusercontent.com/fanyuanqi/1.sh/master/php.ini >/dev/null 2>&1
chmod +644 /etc/php.ini
mkdir /var/lib/php/session >/dev/null 2>&1
chmod -R 777 /var/lib/php/session/
cd  /var/lib/php
...
wget https://raw.githubusercontent.com/fanyuanqi/1.sh/master/config.inc.php >/dev/null 2>&1
chmod -R 777 /var/lib/php/session/
cd /etc/nginx/conf.d
wget https://raw.githubusercontent.com/fanyuanqi/1.sh/master/phpmyadmin.conf >/dev/null 2>&1
systemctl restart nginx.service
echo 
...
echo -e "\033[36m
---------------------------------------------------------
使用方法为，下载openvpn，导入免流配置，连接，输入登录名和密码
...
echo -e "\033[36m2 - 添加一个新用户（请在安装完成后执行）.\033[0m"
echo -e "\033[36m3 - 查看账号列表.\033[0m"
echo -e "\033[36m4 - 删除一个账号.\033[0m"
...
if [[ "$installslect" == "2" ]]
	...
	echo
	echo
 echo -e "\033[34m请设置新账号，默认admin，禁止输入已经存在的账号，否则程序崩溃 \033[0m"
 echo
 echo -n -e "\033[36m请输入账号\033[0m \033[33m【温馨提示:建议修改】\033[0m :"
 read id
 if [[ -z $id ]]
 then
 exit 0;
 fi
 echo -e "\033[34m已设置用户名为：$id\033[0m"
 echo
 echo -e "\033[34m请设置密码(默认 123456)  \033[0m"
 echo
